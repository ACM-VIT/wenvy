...
3. SSH data plane
   - Operational push, pull, and share commands use SSH.
   - A Go SSH gateway sits behind Cloudflare Tunnel for MVP, or Spectrum when public L4 TCP edge is needed.
   - The gateway authenticates SSH keys and evaluates the same role/repo/branch authorization model as the HTTP API.
...
- The Worker and SSH gateway are semi-trusted: they enforce identity, authorization, branch policy, audit, and coordination, but cannot decrypt secret payloads.
